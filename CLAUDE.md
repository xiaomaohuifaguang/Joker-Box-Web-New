...
app/
  ...
  (front)/              # Front route group (URL unaffected)
    ...
    ganDaShi/              # 干大事论坛（列表+详情+发帖+评论，TipTap 富文本，已实现）
      page.tsx + _components/ (ForumInner, PostList, PostDetail, NewPost, CommentSection, CommentThread, RichTextEditor, RichContent, ResizableImage)
...
components/
  ...
  RequireAdmin.tsx      # 登录 + admin + 后台菜单树路由守卫 -> 404 (client)
...
- **干大事论坛** `/ganDaShi`（`<RequirePermission>`）- 三视图（列表/详情/发帖）：**state 驱动 + `window.history.pushState` 同步 URL**（`?thread=id`/`?new=1`/无参=列表，`ForumInner`）。**不用 `router.push` 切 query**——静态导出下同 path 仅改 query 的软导航不可靠（返回按钮偶发无效）；`popstate` 监听同步前进/后退，URL 可分享/刷新还原。**列表**（搜索+分页+帖子卡：标题/digest/作者/时间/浏览量）；**详情**（content 用 `RichContent` = `prose`+`DOMPurify` 渲染 + 嵌套评论：根评论分页 + 展开 `replayCount` 条回复 + 回复表单）；**发帖**（TipTap 编辑器 -> content(HTML)+text(纯文字)）。TipTap（headless+主题 token，starter-kit+link+placeholder+image，`compact` 评论用；图片 base64 内联，canvas 压缩 maxW=1200 JPEG 0.85，>1MB 拒绝，工具栏+粘贴/拖拽自动插入；**图片可缩放/混排**，见下）+ `@tailwindcss/typography`（prose）+ `DOMPurify`（`typeof window` 守卫避 SSR）。删帖（作者/admin，`createBy`）；删评论（admin，评论无 createBy）。接口 `/ganDaShiPost/{queryPage,add,info,remove}` + `/ganDaShiComment/{queryPage,add,remove}`；`usePostPage`/`useComments`。
...
- **动态表单设计器** `/console/form/dynamicForm-manager`（`<RequireAdmin>`）- 两视图（state 驱动 + pushState，同 ganDaShi 模式）：**列表**（搜索+分页+表格+CRUD，name/description/version/status 0草稿/1发布/-1停用，`FormListPanel`）；**设计器** `?design=id|new`（`FormDesigner`，h1 + 三栏：字段库 `FieldPalette` | 画布 `FormCanvas` | 配置 `FieldConfigPanel`）。`useDesignerState`（`designer-state.ts`）管 fields/groups，`toPayload` 存前剥 clientId；fieldId=`crypto.randomUUID()`。
  - **字段库**：`FIELD_REGISTRY`（`fields/registry.tsx`，Record<type, FieldMeta{type/label/group/defaults/Control/hasOptions/hasLength/hasMinMax/hasPattern/hasPlaceholder}>）+ `createField(type,sort)`；19 类型分四组（基础/选择/日期时间/高级）：INPUT/TEXTAREA/NUMBER/SWITCH/SLIDER/RATE/COLOR/SELECT/MULTISELECT/RADIO/CHECKBOX/DATE/TIME/DATETIME/DATERANGE/UPLOAD/CASCADER/MULTICASCADER/TABLE。**v1 未做**：发布/版本切换（联动规则已实现，见下；optionSource 远程已实现，见下）。**DATERANGE**：值=`[start,end]` 字符串数组，`props.withTime`（配置面板「包含时间」开关）切 date-only `yyyy-MM-dd` / 含时间 `yyyy-MM-dd HH:mm`，`DateRangeControl` 双日历+「至」，选日保留已输入时间；校验 end<start。**TABLE**：值=`Record<列key,string>[]` 行对象数组，列定义存字段 `tableColumns: DynamicFormTableColumn[]{key,title}`（配置面板 `TableColumnsDialog` 增删列+key去重），`TableControl` 单元格统一文本输入+增删行，无默认值；联动操作符仅 EMPTY/NOT_EMPTY。
  ...
  - **联动规则**（`linkageRules` 随 add/update/info 走）：**任意嵌套条件树**（AND/OR 节点的 children 可含 CONDITION 或子 AND/OR 组，嵌套 children 不读 id/parentId，后端自动算 id）。9 动作 SHOW/HIDE/REQUIRED/OPTION/VALUE/DISABLED/ENABLED/SET_PATTERN/SET_SPAN，**条件满足才执行、不满足一律回字段原配置**（SHOW/HIDE 不反义）。字段有 `visible` 属性（默认 true，字段配置「默认显示」开关）：false=默认隐藏，配合 SHOW 满足才显示（「选A才显示」=默认隐藏+SHOW；「选A才隐藏」=默认显示+HIDE）。求值引擎 `linkage.ts`（`evalCondition`/`evalNode` 递归：AND=children.every/OR=some/空children=恒真，`evalRule` 对根调 evalNode，`computeFieldState` 纯函数，初始 visible 取字段配置）；预览每字段过 `computeFieldState` 得有效状态（visible/required/disabled/options/pattern/span），隐藏/禁用字段保留值、不校验、不进查看数据；VALUE 边沿触发赋一次（ref 记上次条件结果）。**OPTION 动作=全量替换**：`actionValue` 存完整选项树 `DynamicFormOption[]`（字段配置副本，每项带 visible），命中时 `state.options=actionValue` 整体替换，渲染仍走 `visibleOptions()` 过滤显隐；编辑器复用字段配置选项弹窗，`OptionsEditor visibilityOnly`（仅切显隐、禁增删改），打开带入字段当前 options（含已有显隐）。编辑 UI=右侧「字段配置|联动规则」Tab（`LinkagePanel` 卡片列表+上下移+启停，`ruleSummary` 条件值/动作值经 value→label 映射显 label）+ 宽弹窗 `LinkageRuleEditor`（**递归 ConditionGroupNode** 条件树：AND/OR 切换+行列表，行=单条件或缩进子组；**条件值复用触发字段真实控件**（借 createField 造临时字段，开关/颜色/日期/级联等各用各控件，REGEX 用文本框，IN/NOT_IN 用标签式多选/多选级联；选值列全部选项含 visible=false，显隐是运行态的事）；OPTION 动作用 `OptionVisibilityEditor`；规则名可空；自引用保存时 toast 阻止）。类型 `DynamicFormLinkageRule/Node/ActionType/Condition`（types/dynamic-form.ts）。**删字段级联删引用规则**（`removeField` 同时过滤 `linkageRules`，`ruleReferencesField` 递归查目标/触发）；**选项变更自动同步规则**（`updateField` 检测 options 变化时 `syncRulesOnOptionsChange`：触发条件引用失效 value 剔除该条件 `pruneConditionTree`、VALUE 值失效清 undefined `filterValueByOptions`、OPTION 树以最新 options 为骨架对齐 `reconcileOptionTree`——label 同步/新增默认可见/失效剔除/已有 visible 保留）；**VALUE 循环赋值熔断**（连续 ≥10 次赋值停止 + toast 提示，收敛/reset 复位 `valueChainCount`）。
  - **远程数据源**（`optionSource` 随 add/update/info 走）：选项类字段（SELECT/MULTISELECT/RADIO/CHECKBOX/CASCADER/MULTICASCADER）的 options 可改为运行时从 /joker-box 接口拉取。`DynamicFormField.optionSource{type:"STATIC"|"API",url,method,params,mapping}`（types/dynamic-form.ts；STATIC/不配=手动 options，API=远程）。**取值优先级（关键，已去手动兜底）**：`type==="API"` 完全以远程为准——拉取**成功（含空数组）**用远程结果（空即空，显「暂无可用选项」）；**失败/异常**控件占位显「数据源异常」（禁用，当 disabled 不校验、不进查看数据）；**加载中/依赖未填/地址非法**显「加载中…」。均**不回退**手动 options。`params` 的 value 支持 **`${fieldId}` 占位**：运行时取该表单字段当前值替换（整串占位保留类型，子串做拼接），依赖字段值变化**自动重拉**（级联数据源），依赖有空值则跳过请求。`mapping{listPath,labelPath,valuePath,childrenPath}`：响应映射，`$`=lib/api 剥过的 data 本身，留空走默认 label/value/children，children 递归（级联）；支持 JSONPath 风格 $.data（= data 字段）。`optionSource.ts` 纯函数（`resolvePath`/`collectDeps`/`substituteParams`/`mapOptions`，label/value 强制 String）；`useRemoteOptions(fields,values)` 预览拉取（请求 key=url+method+替换后参数，竞态守卫后发覆盖先发，url 仅允许 `/` 开头不含 `://`），返回 `optionsOf`（成功才给远程 options）+ `statusOf`（ready/error/loading）；渲染前把远程 options 合并进 field 再过 `computeFieldState`（OPTION 未命中=远程，命中=actionValue），数据源状态经 `field.props.__sourceError/__sourceLoading`（运行时注入、保存前 `stripClient` 剥离）传给控件显占位，error 时 `effState` 置 `required:"0",options:[],disabled:true`。**默认值编辑器跟随数据源**：API 字段恒显只读提示「运行时按远程选项选值」（设计态拉不到远程选项，不给设默认值）。**数据源变化清理联动规则（二级确认）**：选项类字段的 `optionSource` 实质变化（手动<->远程切换 / 远程 url/method/params/mapping 改）将清空所有涉及该字段的联动规则（`ruleReferencesField`：目标或触发；SHOW/HIDE 等一并清，一致性代价）。清规则前弹 AlertDialog 二级确认「将清空 N 条规则」：确认=应用配置+清规则，取消=还原（配置不变）。STATIC 选项编辑即时应用（走 `syncRulesOnOptionsChange` 精准同步，不清整条、不确认）；切换数据来源即时确认（点 ToggleGroup 即弹）；API 配置用 draft 暂存（编辑不即时生效），点「确定」才应用+确认（连续编辑不打断）。`updateField` 检测 `optionSource` 实质变化时清规则（不 toast，确认由 UI 完成）。远程选项**不进**联动规则配置（条件值/VALUE/OPTION 仍用手动 `options`）；故 API 字段当触发条件时手动 options 空则条件值配不了（已知限制）。**码表作 API 预设**：选项弹窗「数据来源」切到 API 时默认填码表预设 `url=/code-table/options,method=GET,params={code:""},mapping={listPath:"$.data"}`（用户补 code 即可）；点「自定义」预设清空手填。配置 UI=`OptionsDialog` 顶部 STATIC/API 切换（ToggleGroup），API 表单=预设/url(校验标红)/method/params 键值行(可插 `${fieldId}` 字段引用)/mapping 4 输入；摘要按钮 STATIC 显「N 个选项」、API 显「远程：{url}」或码表显「码表：{code}」。
