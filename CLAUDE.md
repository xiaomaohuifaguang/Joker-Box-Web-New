...
- **动态表单设计器** `/console/form/dynamicForm-manager`（`<RequireAdmin>`）- 两视图（state 驱动 + pushState，同 ganDaShi 模式）：**列表**（搜索+分页+表格+CRUD，name/description/version/status 0草稿/1发布/-1停用，`FormListPanel`）；**设计器** `?design=id|new`（`FormDesigner`，h1 + 三栏：字段库 `FieldPalette` | 画布 `FormCanvas` | 配置 `FieldConfigPanel`）。`useDesignerState`（`designer-state.ts`）管 fields/groups，`toPayload` 存前剥 clientId；fieldId=`crypto.randomUUID()`。
  ...
  - **联动规则**（`linkageRules` 随 add/update/info 走）：**任意嵌套条件树**（AND/OR 节点的 children 可含 CONDITION 或子 AND/OR 组，嵌套 children 不读 id/parentId，后端自动算 id）。9 动作 SHOW/HIDE/REQUIRED/OPTION/VALUE/DISABLED/ENABLED/SET_PATTERN/SET_SPAN，条件满足才执行、不满足按原配置（SHOW/HIDE 反义例外）。求值引擎 `linkage.ts`（`evalCondition`/`evalNode` 递归：AND=children.every/OR=some/空children=恒真，`evalRule` 对根调 evalNode，`computeFieldState` 纯函数）；预览每字段过 `computeFieldState` 得有效状态（visible/required/disabled/options/pattern/span），隐藏/禁用字段保留值、不校验、不进查看数据；VALUE 边沿触发赋一次（ref 记上次条件结果）。**OPTION 动作=可见子集**：`actionValue` 存可见选项的 value `string[]`，命中时 `options=field.options.filter(o=>allow.includes(o.value))`（保留原选项对象含 children/visible），渲染仍走 `visibleOptions()` 过滤显隐——字段选项显隐照旧生效，OPTION 不可新增值；级联按根级整枝显隐。编辑 UI=右侧「字段配置|联动规则」Tab（`LinkagePanel` 卡片列表+上下移+启停）+ 宽弹窗 `LinkageRuleEditor`（**递归 ConditionGroupNode** 条件树：AND/OR 切换+行列表，行=单条件或缩进子组，可加条件/子组/删组；OPTION 动作用 `OptionSubsetPicker` 勾选目标字段已配选项的可见子集，级联只列根级；条件区按触发字段类型渲染值控件+操作符过滤；自引用保存时 toast 阻止）。类型 `DynamicFormLinkageRule/Node/ActionType/Condition`（types/dynamic-form.ts）。**删字段级联删引用规则**（`removeField` 同时过滤 `linkageRules`，`ruleReferencesField` 递归查目标/触发）；**VALUE 循环赋值熔断**（连续 ≥10 次赋值停止 + toast 提示，收敛/reset 复位 `valueChainCount`）。
