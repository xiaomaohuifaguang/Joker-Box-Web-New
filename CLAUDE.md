...
- **动态表单设计器** `/console/form/dynamicForm-manager`（`<RequireAdmin>`）- 两视图（state 驱动 + pushState，同 ganDaShi 模式）：**列表**（搜索+分页+表格+CRUD，name/description/version/status 0草稿/1发布/-1停用，`FormListPanel`）；**设计器** `?design=id|new`（`FormDesigner`，h1 + 三栏：字段库 `FieldPalette` | 画布 `FormCanvas` | 配置 `FieldConfigPanel`）。`useDesignerState`（`designer-state.ts`）管 fields/groups，`toPayload` 存前剥 clientId；fieldId=`crypto.randomUUID()`。
  - **字段库**：`FIELD_REGISTRY`（`fields/registry.tsx`，Record<type, FieldMeta{type/label/group/defaults/Control/hasOptions/hasLength/hasMinMax/hasPattern/hasPlaceholder}>）+ `createField(type,sort)`；20 类型分四组（基础/选择/日期时间/高级）：INPUT/TEXTAREA/NUMBER/SWITCH/SLIDER/RATE/COLOR/SELECT/MULTISELECT/RADIO/CHECKBOX/DATE/TIME/DATETIME/DATERANGE/UPLOAD/CASCADER/MULTICASCADER/TABLE。**v1 未做**：发布/版本切换、optionSource 远程（联动规则已实现，见下）。**DATERANGE**：值=`[start,end]` 字符串数组，`props.withTime`（配置面板「包含时间」开关）切 date-only `yyyy-MM-dd` / 含时间 `yyyy-MM-dd HH:mm`，`DateRangeControl` 双日历+「至」，选日保留已输入时间；校验 end<start。**TABLE**：值=`Record<列key,string>[]` 行对象数组，列定义存字段 `tableColumns: DynamicFormTableColumn[]{key,title}`（配置面板 `TableColumnsDialog` 增删列+key去重），`TableControl` 单元格统一文本输入+增删行，无默认值；联动操作符仅 EMPTY/NOT_EMPTY。
