...
- **动态表单设计器** `/console/form/dynamicForm-manager`（`<RequireAdmin>`）- 两视图（state 驱动 + pushState，同 ganDaShi 模式）：**列表**（搜索+分页+表格+CRUD，name/description/version/status 0草稿/1发布/-1停用，`FormListPanel`）；**设计器** `?design=id|new`（`FormDesigner`，h1 + 三栏：字段库 `FieldPalette` | 画布 `FormCanvas` | 配置 `FieldConfigPanel`）。`useDesignerState`（`designer-state.ts`）管 fields/groups，`toPayload` 存前剥 clientId；fieldId=`crypto.randomUUID()`。
  ...
  - **远程数据源**（`optionSource` 随 add/update/info 走）：选项类字段（SELECT/MULTISELECT/RADIO/CHECKBOX/CASCADER/MULTICASCADER）的 options 可改为运行时从 /joker-box 接口拉取。`DynamicFormField.optionSource{type:"STATIC"|"API",url,method,params,mapping}`（types/dynamic-form.ts；STATIC/不配=手动 options，API=远程）。**取值优先级（关键）**：`type==="API"` 以远程为准——拉取**成功（含空数组）**用远程结果（空即空，显「暂无可用选项」，**不回退**手动）；**失败/未加载**回退手动 `options` 兜底（切 API 时保留手动 options 作兜底）。`params` 的 value 支持 **`${fieldId}` 占位**：运行时取该表单字段当前值替换（整串占位保留类型，子串做拼接），依赖字段值变化**自动重拉**（级联数据源），依赖有空值则跳过请求走手动兜底。`mapping{listPath,labelPath,valuePath,childrenPath}`：响应映射，`$`=lib/api 剥过的 data 本身，留空走默认 label/value/children，children 递归（级联）。`optionSource.ts` 纯函数（`resolvePath`/`collectDeps`/`substituteParams`/`mapOptions`，label/value 强制 String）；`useRemoteOptions(fields,values)` 预览拉取（请求 key=url+method+替换后参数，竞态守卫后发覆盖先发，url 仅允许 `/` 开头不含 `://`），渲染前把远程 options 合并进 field 再过 `computeFieldState`（OPTION 未命中=远程，命中=actionValue）。**码表作 API 预设**：选项弹窗「数据来源」切 API，预设「码表选项」一键填 `url=/code-table/options,method=POST,params={code:""}`，用户补 code。配置 UI=`OptionsDialog` 顶部 STATIC/API 切换（ToggleGroup），API 表单=预设/url(校验标红)/method(默认POST)/params 键值行(可插 `${fieldId}` 字段引用)/mapping 4 输入；摘要按钮 STATIC 显「N 个选项」、API 显「远程：{url}」或码表显「码表：{code}」。
