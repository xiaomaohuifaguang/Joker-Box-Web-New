...
export async function queryOrgPage(params: {
  ...
  const { data } = await api.post<Page<Org>>("/org/queryPage", {
    body: {
      parentId: params.parentId != null ? String(params.parentId) : "-1",
      size: params.size,
...
export async function addOrg(params: {
  ...
  await api.post<unknown>("/org/add", { body: params });
}

// 删除：POST /org/delete，判断 code 即可（无 data）。
export async function removeOrg(id: number): Promise<void> {
  await api.post<unknown>("/org/delete", { body: { id } });
}
...
export async function updateOrg(params: {
  ...
  await api.post<unknown>("/org/update", { body: params });
}

// 详情：POST /org/info。
export async function getOrgInfo(id: number): Promise<OrgDetail> {
  const { data } = await api.post<OrgDetail>("/org/info", { body: { id } });
  return data;
}
